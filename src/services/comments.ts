...
export const getCommentsHandle = async (commentId?: string, novelId?: string, chapterId?: string, chapterNumber?: number, page?: number) => {
    try {
        const commentsRes = await axios.get(`${BACKEND_URL}/api/comments/get?commentId=${commentId || ''}&novelId=${novelId || ''}&chapterId=${chapterId || ''}&chapterNumber=${chapterNumber || ''}&page=${page || ''}`);
...
export const addCommentHandle = async (data : { receiverId?: string, novelId: string, chapterId?: string, chapterNumber?: number, commentText: string, token: string, senderName: string }) => {
    try {
        ...
        const commentsRes = await axios.post(`${BACKEND_URL}/api/comments/add`, {
...
export const addReplyCommentHandle = async (data: { parentId: string, novelId: string, senderName: string, receiverId: string, commentText: string, token: string }) => {
    try {
        ...
        const commentResponse = await axios.post(`${BACKEND_URL}/api/comments/add/reply`, {
            parentId,
            novelId,
            senderName,
            receiverId,
...
export const getReplyCommentsHandle = async (commentId: string, page: number) => {
    try {
        const commentsRes = await axios.get(`${BACKEND_URL}/api/comments/get/reply/${commentId}?page=${page}`);
...
export const destroyCommentHandle = async (commentId: string, token: string) => {
    try {
        ...
        const commentRes = await axios.delete(`${BACKEND_URL}/api/comments/destroy/${commentId}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
...
export const destroyReplyCommentHandle = async (commentId: string, token: string) => {
    try {
        const commentRes = await axios.delete(`${BACKEND_URL}/api/comments/destroy/reply/${commentId}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
...
export const getCommentsNotifyHandle = async (page: number, token: string) => {
    try {
        const commentsRes = await axios.get(`${BACKEND_URL}/api/comments/get/notify?page=${page}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
...
export const readCommentNotifyHandle = async (commendId: string, token: string) => {
    try {
        const commentsRes = await axios.post(`${BACKEND_URL}/api/comments/add/read/notify/${commendId}`, {}, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        });
